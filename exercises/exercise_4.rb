...
mens_stores.each do |store|
  puts "Store location: #{store.name} Annual revenue: #{store.annual_revenue}"
end

womens_budget_stores = Store.where("womens_apparel = ? AND annual_revenue < ?", true, 1000000)
puts "women's affordable clothing stores:"
womens_budget_stores.each do |store|
  puts "Store location: #{store.name} Annual revenue: #{store.annual_revenue}"
end
